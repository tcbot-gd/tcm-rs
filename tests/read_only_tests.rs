...
fn test_v1_read_only() {
    ...
    println!("  Input count: {}", replay.inputs.len());
        ...
        println!("  Input {}: frame={}, type={:?}", i, input_cmd.frame, input_cmd.input);
        assert!(input_cmd.frame < 10_000_000, "Frame number seems unreasonable");
...
fn test_v2_long_read_only() {
    ...
    for input_cmd in &replay.inputs {
        assert!(input_cmd.frame < 100_000_000, "Frame number seems unreasonable");
    ...
    println!("  Bugpoint inputs: {}", bugpoint_count);
        ...
        assert!(replay.inputs.len() > 10, "Long file should have many inputs");
}
...
fn test_file_headers() {
    ...
    for file_name in ["examples/restartv1.tcm", "examples/restartv2.tcm", "examples/longv2.tcm"] {
        ...
        assert_eq!(header, TCBOT_HEADER, "File {} has incorrect header", file_name);
...
fn test_metadata_parsing() {
        ...
        assert!(file_bytes.len() > 16 + 0x40, "V1 file too short for metadata");
        
        let meta_bytes = &file_bytes[16..16 + 0x40];
        let meta = MetaV1::from_bytes(meta_bytes);
        
        println!("V1 metadata: TPS={}, append_counter={}", meta.tps(), meta.append_counter);
        assert!(meta.tps() > 0.0 && meta.tps() < 10000.0, "V1 TPS seems unreasonable");
    }
        ...
        assert!(file_bytes.len() > 16 + 0x40, "V2 file too short for metadata");
        
        let meta_bytes = &file_bytes[16..16 + 0x40];
        let meta = MetaV2::from_bytes(meta_bytes);
        
        println!("{} metadata: TPS={}, uses_dt={}, rng_seed={:?}, append_counter={}", 
                 file_name, meta.tps(), meta.uses_dt(), meta.rng_seed(), meta.append_counter);
        assert!(meta.tps() > 0.0 && meta.tps() < 10000.0, "V2 TPS seems unreasonable");
    }
}
