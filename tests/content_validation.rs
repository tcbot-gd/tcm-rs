...
fn test_v1_content_validation() {
    ...
    assert!(replay.meta.rng_seed().is_none(), "V1 should not have RNG seed");
    assert!(!replay.meta.is_rng_seed_set(), "V1 should not have RNG seed set");
        ...
        match &input_cmd.input {
            Input::Vanilla(vanilla) => {
                                assert!(matches!(vanilla.button, PlayerButton::Jump | PlayerButton::Left | PlayerButton::Right));
            }
            Input::Restart(restart) => {
                                assert!(matches!(restart.restart_type, RestartType::Restart | RestartType::RestartFull | RestartType::Death));
                                assert!(restart.new_seed.is_none(), "V1 restarts should not have new seeds");
            }
            Input::Tps(_) | Input::Bugpoint(_) => {
                ...
                assert!(input_cmd.frame < 1_000_000, "Frame number seems unreasonably high");
    }

    println!("V1 file validation passed with {} inputs", replay.inputs.len());
}
...
fn test_v2_restart_content_validation() {
    ...
    let replay = replay_template
        ...
        assert!(replay.meta.tps() > 0.0, "TPS should be positive");
    assert!(replay.meta.tps_dt() > 0.0, "TPS delta time should be positive");
        ...
        match &input_cmd.input {
            ...
            Input::Bugpoint(_) => {
                ...
                assert!(input_cmd.frame < 10_000_000, "Frame number seems unreasonably high");
    }

    println!("V2 restart file validation passed with {} inputs", replay.inputs.len());
}
    ...
    for input_cmd in &replay.inputs {
        match &input_cmd.input {
            ...
            Input::Bugpoint(_) => {
                ...
                assert!(input_cmd.frame < 100_000_000, "Frame number seems unreasonably high");
    }

    println!("V2 long file validation passed with {} inputs ({} vanilla, {} restart, {} tps, {} bugpoint)", 
             replay.inputs.len(), vanilla_count, restart_count, tps_count, bugpoint_count);
    
        assert!(replay.inputs.len() > 10, "Long file should have more than 10 inputs");
}
...
fn test_frame_ordering() {
    for (file_name, is_v1) in [
        ...
        assert!(example_path.exists(), "Example file {} not found", file_name);

        let replay = if is_v1 {
            let mut file = File::open(example_path).expect("Failed to open file");
            let replay_template = Replay::<MetaV1>::new_empty(240.0);
            let replay = replay_template.deserialize(&mut file).expect("Failed to deserialize file");
        ...
        for input_cmd in &inputs {
                ...
                assert!(
                    ...
                    file_name, input_cmd.frame, last_frame
                );
                last_frame = input_cmd.frame;
            }
        }
